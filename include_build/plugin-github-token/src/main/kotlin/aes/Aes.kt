...
class Aes {
    ...
    fun encrypt(plain: ByteArray, key: ByteArray) =
        encrypt(plain, AesRawBytesSecret(key))

    fun decrypt(cipher: ByteArray, key: ByteArray) =
        decrypt(cipher, AesRawBytesSecret(key))

    fun encrypt(plain: ByteArray, key: AesSecret): ByteArray {
    ...
    fun decrypt(cipher: ByteArray, key: AesSecret): ByteArray {
        val keyBytes: ByteArray = key.toByteArray16()
    ...
    internal fun encrypt16Bytes(plain: ByteArray, key: ByteArray): ByteArray {
        expandKey(key)
        var cipher = plain xor getRoundKey(0)
    ...
    internal fun decrypt16Bytes(cipher: ByteArray, key: ByteArray): ByteArray {
        expandKey(key)
